    ...
    E: CustomMsg,
{
    ...
    pub achieved_token_count: Item<'a, u64>,
    ...
    Q: CustomMsg,
{
    fn new(
        ...
        achieved_token_count_key: &'a str,
    ) -> Self {
        ...
        Self {
            ...
            achieved_token_count:Item::new(achieved_token_count_key),
    ...
    pub fn achieved_token_count(&self, storage: &dyn Storage) -> StdResult<u64> {
        Ok(self.achieved_token_count.may_load(storage)?.unwrap_or_default())
    }

    pub fn increment_achieved_token(&self, storage: &mut dyn Storage) -> StdResult<u64> {
        let val = self.achieved_token_count(storage)? + 1;
        self.achieved_token_count.save(storage, &val)?;
        Ok(val)
    }
}
