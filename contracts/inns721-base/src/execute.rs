    ...
    Q: CustomMsg,
{
    pub fn mint(
        ...
        // if achieved_token_count > max_supply stop mint
        let contract_info = self.contract_info.load(deps.storage)?;
        let count = self.achieved_token_count(deps.storage)?;
        if contract_info.max_supply <= count {
            return Err(ContractError::QuantityOverflow {});
        }
        ...
        self.increment_achieved_token(deps.storage)?;
